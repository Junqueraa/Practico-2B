...
public class App {
    ...
    public static void main(String[] args) {
        ...
        get("/materias/new", (req, res) -> {
            Map<String, Object> model = new HashMap<>();
            // Pasamos mensajes vacíos para que Mustache no los muestre al principio
            model.add("successMessage", req.session().attribute("successMessage"));
            model.add("errorMessage", req.session().attribute("errorMessage"));
            
            // Limpiamos los mensajes de la sesión para que no se queden fijos
        ...
        // 2. Procesar el envío del formulario (Guardar en Base de Datos)
        post("/materias/new", (req, res) -> {
            // Abrimos la conexión a la base de datos (por las dudas si no se abrió en el before)
            DBConfigSingleton.getInstance().openConnection();
            try {
                // Capturamos los datos que el usuario escribió en el HTML
                String codigoStr = req.queryParams("codigo_mat");
                String nombre = req.queryParams("nombre_mat");

                int codigo = Integer.parseInt(codigoStr);

                // Creamos la nueva materia usando ActiveJDBC
                com.is1.proyecto.models.Materia nuevaMateria = new com.is1.proyecto.models.Materia();
                nuevaMateria.setCodigo(codigo);
                nuevaMateria.setNombre(nombre);
                nuevaMateria.saveIt(); // Lo guarda físicamente en la tabla 'materias'

                // Guardamos mensaje de éxito y relanzamos la pantalla
                req.session().attribute("successMessage", "¡Materia '" + nombre + "' guardada con éxito!");
                res.redirect("/materias/new");
                return null;

            } catch (NumberFormatException e) {
                req.session().attribute("errorMessage", "El código de la materia debe ser un número válido.");
                res.redirect("/materias/new");
                return null;
            } catch (Exception e) {
                req.session().attribute("errorMessage", "Error al guardar la materia: " + e.getMessage());
                res.redirect("/materias/new");
                return null;
            } finally {
                // Cerramos la conexión para no dejar hilos abiertos
